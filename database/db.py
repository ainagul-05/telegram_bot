# import sqlite3
from database.queries import create_products_table, create_orders_table, create_products_detaiil_table ,  create_orders_detail_table, insert_product, insert_order, insert_product_detail, insert_order_detail
import aiosqlite
...
async def init_db():
    async with aiosqlite.connect(path_db) as conn:
        ...
        await conn.execute(create_products_detaiil_table)
        await conn.commit()
...
async def init_orders_db():
    async with aiosqlite.connect(path_db) as conn:
        ...
        await conn.execute(create_orders_detail_table)
        await conn.commit()
...
async def add_order_db(size, stuffing, address, order_id, status, photo):
    async with aiosqlite.connect(path_db) as conn:
        await conn.execute(insert_order, (size, stuffing, address))
        await conn.execute(insert_order_detail, (order_id, status, photo))
        await conn.commit()
