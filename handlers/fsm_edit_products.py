...
async def etit_start(call: CallbackQuery,state:FSMContext):
...
async def edit_save(message: Message, state : FSMContext):
    ...
    if data['field'] == 'price' and not message.text.isdigit():
        await message.answer('Цена товара должен быть числом . Попробуйте еще раз')
        return
    else:
        await db.update_product_db(field=data['field'], value=message.text, product_id=data['product_id'])
